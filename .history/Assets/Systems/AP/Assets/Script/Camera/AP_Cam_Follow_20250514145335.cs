...
{
    if (playerModel == null || characterMovementScript == null || characterMovementScript.objCamera == null) return;
    ...
    // Use rigidbody for rotation if available and matches rbBodyCharacter
    if (playerRigidbody != null && playerRigidbody == characterMovementScript.rbBodyCharacter)
    {
        // Skip manual rotation if characterMovement is handling it
        // Ensure rotation is consistent with characterMovement's rbBodyCharacter
        if (ingameGlobalManager.instance.b_DesktopInputs)
        {
            // For desktop, rely on characterMovement's bodyRotation
            return;
        }
        else
        {
            // For mobile, align with bodyRotationMobile
    ...
    {
        // If no rigidbody or different from rbBodyCharacter, use transform directly
