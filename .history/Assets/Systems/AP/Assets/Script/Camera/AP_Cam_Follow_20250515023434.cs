...
public class AP_Cam_Follow : MonoBehaviour
{
    ...
    void UpdatePlayerFacing()
    {
        ...
        Debug.Log(characterMovementScript.GetMouseYInput() + " " + characterMovementScript.GetMouseXInput());
        // Smoothly move to target position
        mianCam.position = target.position;


        _xRotation = Mathf.Clamp(
            _xRotation - characterMovementScript.GetMouseYInput() * 55f * Time.smoothDeltaTime,
            UpperLimit,
            BottomLimit);
        ...
        playerRigidbody.MoveRotation(
            ...
            Quaternion.Euler(0, characterMovementScript.GetMouseXInput() * 55f * Time.smoothDeltaTime, 0));
    }
