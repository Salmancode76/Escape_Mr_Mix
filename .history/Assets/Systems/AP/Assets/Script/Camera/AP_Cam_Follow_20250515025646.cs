...
void UpdatePlayerFacing()
{
    ...
    // Set player model's Y rotation to match camera's Y rotation
    ...
    // Account for parent rotation if playerModel has a parent
    Quaternion parentRotation = playerModel.parent != null ? playerModel.parent.rotation : Quaternion.identity;
    Quaternion finalRotation = parentRotation * targetRotation;

    // Set player model rotation directly to match camera
    playerModel.rotation = finalRotation;
}
