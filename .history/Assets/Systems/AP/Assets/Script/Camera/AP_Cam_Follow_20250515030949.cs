...
void UpdatePlayerFacing()
{
    ...
    // Vertical (X-axis) camera tilt
    _xRotation = Mathf.Clamp(
        _xRotation - characterMovementScript.GetMouseYInput() * playerTurnSpeed * Time.smoothDeltaTime,
        UpperLimit,
        BottomLimit);

    target.localRotation = Quaternion.Euler(_xRotation, 0f, 0f);

    // Match Y-axis rotation exactly
    float cameraYRotation = characterMovementScript.objCamera.localEulerAngles.y;
    Quaternion exactYRotation = Quaternion.Euler(0f, cameraYRotation, 0f);

    // Force player model rotation to match camera Y
    playerModel.rotation = exactYRotation;

    // If using Rigidbody, also apply directly (avoid MoveRotation for exact match)
    if (playerRigidbody != null)
    {
        playerRigidbody.rotation = exactYRotation;
    }
}
