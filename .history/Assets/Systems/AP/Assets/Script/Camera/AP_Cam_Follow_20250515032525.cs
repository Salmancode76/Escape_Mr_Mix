...
void UpdatePlayerFacing()
{
    ...
    // --- Force camera and player to have same Y rotation ---
    float yRotation = characterMovementScript.objCamera.localEulerAngles.y;

    // Directly apply to player model
    playerModel.rotation = Quaternion.Euler(0f, yRotation, 0f);

    // Directly apply to player Rigidbody (if exists)
    if (playerRigidbody != null)
    {
        playerRigidbody.rotation = Quaternion.Euler(0f, yRotation, 0f);
    }

    // Directly apply to camera parent (if needed)
    transform.rotation = Quaternion.Euler(_xRotation, yRotation, 0f);
}
