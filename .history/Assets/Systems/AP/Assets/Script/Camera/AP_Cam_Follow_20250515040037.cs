...
public class AP_Cam_Follow : MonoBehaviour
{
    ...
    [SerializeField] float rotationSpeed = 15f; // Adjust this value in the Inspector
...
public void UpdatePlayerRotation()
    {
        ...
        float mouseXInput = characterMovementScript.GetMouseXInput();
        ...
        if (Mathf.Abs(mouseXInput) > 0.01f)
        {
            ...
            {
                // Smooth rotation using RotateTowards
                Quaternion currentRotation = playerRigidbody.rotation;
                Quaternion smoothedRotation = Quaternion.RotateTowards(
                    currentRotation,
                    targetRotation,
                    rotationSpeed * Time.fixedDeltaTime * 100 // Convert speed to degrees/second
                );
            ...
            {
                // Smooth rotation for non-physics objects
                Quaternion currentRotation = playerModel.rotation;
                Quaternion smoothedRotation = Quaternion.RotateTowards(
                    currentRotation,
                    targetRotation,
                    rotationSpeed * Time.deltaTime * 100 // Convert speed to degrees/second
                );
                
                playerModel.rotation = smoothedRotation;
            }
        }
    }

    // Change this to FixedUpdate for physics-based rotation
    void FixedUpdate()
    {
        UpdatePlayerRotation();
    }

// Add this to your existing Update method or create a new one if needed
    void Update()
    {
