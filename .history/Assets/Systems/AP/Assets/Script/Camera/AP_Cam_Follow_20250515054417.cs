...
public class AP_Cam_Follow : MonoBehaviour
{
    ...
    //     //     Quaternion.Euler(0, characterMovementScript.GetMouseXInput() * playerTurnSpeed * Time.smoothDeltaTime, 0));
    // }
    ...
    void SyncCameraRotation()
    {
        ...
        // // Debug.Log(playerRigidbody.rotation);
        // // Quaternion deltaRotation = Quaternion.Euler(0f, mouseX * MouseSensitivity * Time.smoothDeltaTime, 0f);
        // // playerRigidbody.MoveRotation(playerRigidbody.rotation * deltaRotation);

        // // Apply camera Y to player
        // playerRigidbody.MoveRotation(targetRotation);

        // Rotate only camera X (up/down)
        Vector3 headEuler = Head.rotation.eulerAngles;
        transform.rotation = Quaternion.Euler(headEuler.x, 0f, 0f);

        // Rotate player only with mouse X input
        float mouseX = characterMovementScript.GetMouseXInput();
        Quaternion deltaRotation = Quaternion.Euler(0f, mouseX * MouseSensitivity * Time.smoothDeltaTime, 0f);
        playerRigidbody.MoveRotation(playerRigidbody.rotation * deltaRotation);
