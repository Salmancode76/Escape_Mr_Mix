...
public class focusCamEffect : MonoBehaviour {
    ...
    if (Head == null) {
        Debug.LogWarning("No GameObject with tag 'Head' found");
    }
    ...
    if (rbCharacter == null) {
        Debug.LogError("Player GameObject has no Rigidbody component");
    }

    // Get character movement component and its refHead
    var charMovement = character.GetComponent<characterMovement>();
    if (charMovement == null) {
        Debug.LogError("Player GameObject has no characterMovement component");
        return;
    }

    if (charMovement.refHead == null) {
        Debug.LogError("characterMovement.refHead is not assigned");
        return;
    }

    objRef = charMovement.refHead;
}
