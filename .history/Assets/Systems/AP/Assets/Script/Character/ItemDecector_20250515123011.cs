...
public class ItemDecector : MonoBehaviour {
	...
	void Update() {
	if(canvas == null) canvas = ingameGlobalManager.instance.canvasPlayerInfos;
	if (startPoint && canvas) {
		for (var i = 0; i < canvas.gameobjectList.Count; i++) {

			// Skip this index if other lists don't have enough items
			if (i >= canvas.objrefList.Count || i >= canvas.objVisible.Count)
				continue;
				...
				if (canvas.objrefList[i].CompareTag("InteractObject")) {
					if (hit.transform.gameObject != canvas.gameobjectList[i])
						canvas.objVisible[i] = false;
							 ...
							 && ObjectInLayer13.GetComponent<_OrderInLayer>().intOrderInLayer == canvas.gameobjectList[i].GetComponent<_OrderInLayer>().intOrderInLayer
