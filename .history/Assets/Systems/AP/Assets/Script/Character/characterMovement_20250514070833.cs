...
public class characterMovement : MonoBehaviour 
{
    // Reference to your PlayerController
    public PlayerController playerController;

    // Maintain these references but disable their functionality
    public Rigidbody rbBodyCharacter;
    public Transform objCamera;
    public Transform refHead;
    public CapsuleCollider charCol;

    // Keep these parameters but make them reference PlayerController values
    ...
    private void Start()
    {
        ...
        // Initialize references from PlayerController
        if (playerController != null)
        {
            mouseSensitivity = playerController.MouseSensitivity;
            crouchSpeed = playerController.AnimBlendSpeed;
            jumpForce = playerController.JumpFactor / 100f; // Adjusted for scaling
            speedMultiplier = playerController._runSpeed;
        }
    }
    ...
    private void Update()
    {
        ...
        // Update values from PlayerController
        allowCrouch = playerController.InputEnabled;
        b_AllowJump = playerController.InputEnabled;

        // Use PlayerController's grounded state
    ...
    private void FixedUpdate()
    {
        ...
        // Use PlayerController's movement state
    ...
    #region Modified Functionality
    private void HandleCrouch()
    {
        if (!allowCrouch || !playerController.InputEnabled) return;

        // Use PlayerController's crouch state
        if (playerController.IsCrouching)
        {
            if (b_Crouch && AP_CheckIfPlayerCanStopCrouching() || !b_Crouch)
            {
                b_Crouch = !b_Crouch;
                playerController.SetCrouchState(b_Crouch);
            }
        }
    }
    ...
    {
        if (!b_AllowJump || !playerController.InputEnabled) return;

        if (playerController.IsJumping && !b_IsJumping && playerController.IsGrounded)
        {
            StartCoroutine(Jump());
        }
    }
    ...
    private void HandleCrouchPhysics()
    {
        ...
        if (b_Crouch && transform.localScale.y != targetScaleCrouch)
        {
            transform.localScale = Vector3.MoveTowards(transform.localScale,
                new Vector3(transform.localScale.x, targetScaleCrouch, transform.localScale.z),
                Time.deltaTime * crouchSpeed);
        }
        else if (!b_Crouch && transform.localScale.y != refScaleCrouch)
        {
            transform.localScale = Vector3.MoveTowards(transform.localScale,
                new Vector3(transform.localScale.x, refScaleCrouch, transform.localScale.z),
                Time.deltaTime * crouchSpeed);
        }
    }
    ...
    private void AP_ApplyGravity()
    {
        ...
        {
            charCol.material = pIce;
            rbBodyCharacter.AddForce(Physics.gravity * playerController.GravityMultiplier, ForceMode.Acceleration);
        }
    }
    #endregion
    ...
    private bool AP_CheckIfPlayerCanStopCrouching()
    {
        return !Physics.Raycast(transform.position + Vector3.up * 0.1f, 
            Vector3.up, 
            playerController.CrouchCheckHeight, 
            layerCheckCrouch);
    }
    #endregion

    // Removed all movement and camera rotation functionality
    // Removed bodyMovement, bodyRotation, cameraRotation, mobile input handling
    // Removed velocity manipulation and direct physics handling
}