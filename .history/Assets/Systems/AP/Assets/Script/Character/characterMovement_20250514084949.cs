...
//--> Prevent Mac bug with the cursor lockstate
    ...
    // Skip if no animator or input is disabled
    if (!_hasAnimator) return;

    // Set up movement direction based on camera orientation
    ...
    float targetSpeed = isRunning ? characterSpeed : characterSpeed;
    if (b_Crouch) targetSpeed = characterSpeed * 0.75f; // Slower speed when crouching
    
    // Calculate movement direction based on input type
    ...
    if (ingameGlobalManager.instance.b_Joystick) {
        ...
        if (b_TouchLayer12_17 && !isOnFloor) {
            // Do nothing when touching layer 12/17 and not on floor
        } else {
    ...
    // Determine if we should apply movement forces
    if (isOnFloor) {
        if (currentAngle >= 180 - MaxAngle) {
            // Apply movement forces
            rbBodyCharacter.AddForceAtPosition(Direction * targetSpeed * currentSpeedMultiplier, addForceObj.transform.position, ForceMode.Force);
            
            // Animation velocity blending
            if (_hasAnimator) {
        ...
        // if (_hasAnimator && isMoving) {
        //     HandleFootsteps(isMoving);
        // }
    } else {
        // Apply movement forces with less control when in air
        rbBodyCharacter.AddForceAtPosition(Direction * targetSpeed * currentSpeedMultiplier, addForceObj.transform.position, ForceMode.Force);
