...
public class characterMovement : MonoBehaviour {
    ...
    private Vector2 _currentVelocity; // Already exists, reused for animation blending
    [SerializeField] private float AnimBlendSpeed = 8.9f; // From PlayerController
    [SerializeField] private Transform playerModel; // Reference to the GameObject with the Animator
    ...
    // Footstep variables (from PlayerController, adapted)
    [SerializeField] private AudioClip[] walkClipsArray; // Assign in Inspector
    ...
    // Jump audio (from PlayerController)
   ...
   //float mouseInputY = 0;
    ...
    private void Start()
    {
        ...
        if (_hasAnimator)
        {
            _animator.updateMode = AnimatorUpdateMode.Normal; // Ensure Animator doesn't interfere with physics
    ...
    private void FixedUpdate()
    {
        ...
        if (allowCrouch)
        {
            ...
            // Update grounded state for animations (from PlayerController’s SampleGround)
        if (_hasAnimator)
        {
            bool wasGrounded = _animator.GetBool(_groundHash);
            _animator.SetBool(_groundHash, isOnFloor);
            if (isOnFloor && !wasGrounded && hasJumped && !hasPlayedLanding)
            {
                // Play landing sound (from PlayerController)
                if (landingClip != null)
                {
                    // Assuming a SoundFXManager or similar exists; adjust as needed
...
private void bodyRotation()
{
    ...
    if (ingameGlobalManager.instance.mouseWaitUnitlFirstMouseMove)
    {
        ...
        if (!ingameGlobalManager.instance.b_Joystick)
        {
            ...
            Debug.Log($"Mouse X Input: {mouseInputX}"); // Debug input
            if (Mathf.Abs(mouseInputX) > 0.01f) // Small threshold to avoid noise
            {
                tmpXAxis = mouseInputX * sensibilityMouse * ingameGlobalManager.instance.inputListOfFloatKeyboardButton[mouseInGameSensibilty];
                turnAngle = tmpXAxis * Time.deltaTime;
                objCamera.Rotate(0, turnAngle, 0, Space.World); // Rotate camera globally
                playerModel.Rotate(0, turnAngle, 0, Space.World); // Sync player model
            }
        }
        // Joystick Input
        else if (ingameGlobalManager.instance.b_Joystick)
        {
            float joyHorizontal = Input.GetAxis(ingameGlobalManager.instance.inputListOfStringGamepadAxis[JoystickHorizontalAxisCam]);
            Debug.Log($"Joystick Horizontal: {joyHorizontal}"); // Debug input
            if (Mathf.Abs(joyHorizontal) > 0.1f) // Deadzone to avoid drift
            {
                float rotationSpeed = animationCurveJoystick.Evaluate(Mathf.Abs(joyHorizontal)) * 
                                     ingameGlobalManager.instance.inputListOfFloatGamepadButton[gamepadLookInGameSensibilty] * 
                                     sensibilityJoystick * 100f * Time.deltaTime;
                turnAngle = joyHorizontal * rotationSpeed * returnInvertJoystickAxis();
                objCamera.Rotate(0, turnAngle, 0, Space.World);
                playerModel.Rotate(0, turnAngle, 0, Space.World);
            }
        }
        ...
        if (_hasAnimator)
        {
            _animator.SetFloat(_turnAngleHash, turnAngle / Time.deltaTime); // Optional: for turn animations
        }
    }

    // Debug camera rotation
    Debug.Log($"Camera Y Rotation: {objCamera.eulerAngles.y}");
}
...
//--> Prevent Mac bug with the cursor lockstate
    ...
    // Modified bodyMovement to include animation logic (adapted from PlayerController’s Move)
    void bodyMovement()
    {
        ...
        // Set target speed based on state (adapted from PlayerController)
        float targetSpeed = characterSpeed * currentSpeedMultiplier; // Uses run multiplier
        if (b_Crouch) targetSpeed = characterSpeed * 0.75f; // Slower when crouching
        if (XAxis == 0 && YAxis == 0 && !b_MoveRight && !b_MoveLeft && !b_MoveForward && !b_MoveBackward &&
            (!mobileLeftJoystickToMove || mobileLeftJoystickToMove.inputVector == Vector3.zero)) targetSpeed = 0;
        ...
        // Animation logic (from PlayerController)
        bool isMoving = moveInput != Vector2.zero;

        if (isOnFloor)
        {
            // Smoothly blend animation velocities (from PlayerController)
            ...
            // Apply movement forces (existing logic)
            if (currentAngle >= 180 - MaxAngle)
            {
                rbBodyCharacter.AddForceAtPosition(Direction * targetSpeed, addForceObj.transform.position, ForceMode.Force);
            }

            // Apply braking force (existing logic)
            ...
            // Handle footsteps (from PlayerController, adapted)
            if (_hasAnimator && isMoving && !b_Crouch && !b_IsJumping)
            {
        ...
        {
            // Air movement (existing logic)
            rbBodyCharacter.AddForceAtPosition(Direction * targetSpeed, addForceObj.transform.position, ForceMode.Force);

            // Apply air resistance (from PlayerController)
            if (_hasAnimator)
            {
                rbBodyCharacter.AddForce(transform.TransformVector(new Vector3(_currentVelocity.x * 0.8f, 0, _currentVelocity.y * 0.8f)), ForceMode.VelocityChange);
                nextFootstepTime = Time.time + footstepInterval; // Reset footstep timer
            }
        }

        // Cap maximum velocity (existing logic)
    ...
    // Footstep handling (from PlayerController)
    // private void HandleFootsteps(bool isMoving)
    // {
    //     if (!isMoving || !isOnFloor || b_Crouch || b_IsJumping) return;

    //     float effectiveInterval = isRunning ? footstepInterval / 1.5f : footstepInterval;
    ...
    //     // Play sound (adjust to your audio system)
    //     // SoundFXManager.instance.playSoundFXClip(clipToPlay, transform, 1f);
    //     Debug.Log("Footstep sound played"); // Placeholder; replace with actual audio call
    // }
    ...
    //     {
    //         int r = Random.Range(i, seq.Count);
	...
	public void CamRotationMobile()
{
    ...
    {
        // Virtual Joystick (Right Stick)
        float virtualJoyHorizontal = mobileToystickController.inputVector.x;
        Debug.Log($"Mobile Joystick Horizontal: {virtualJoyHorizontal}"); // Debug input
        if (Mathf.Abs(virtualJoyHorizontal) > 0.1f) // Deadzone
        {
            float rotationSpeed = animationCurveMobile.Evaluate(Mathf.Abs(virtualJoyHorizontal)) * 
                                 virtualJoyHorizontal * sensibilityMobile * Time.deltaTime;
            objCamera.Rotate(0, rotationSpeed, 0, Space.World);
            playerModel.Rotate(0, rotationSpeed, 0, Space.World); // Sync player model
        }
        ...
        mouseY = Mathf.Clamp(mouseY, minimum, maximum);
        objCamera.localEulerAngles = new Vector3(mouseY, objCamera.localEulerAngles.y, 0);
    }
    else
    {
        ...
        for (int i = 0; i < Input.touchCount; ++i)
        {
            ...
            {
                float swipe = (Input.GetTouch(i).position - arrStartPos[i]).magnitude;
                ...
                {
                    // Horizontal rotation
                    float rotationSpeed = touchDeltaPosition.x * mobileSpeedRotation * Time.deltaTime;
                    Debug.Log($"Touch Horizontal Delta: {touchDeltaPosition.x}, Rotation: {rotationSpeed}"); // Debug input
                    objCamera.Rotate(Vector3.up * rotationSpeed, Space.World);
                    playerModel.Rotate(Vector3.up * rotationSpeed, Space.World); // Sync player model
	...
	private void bodyRotationMobile()
{
    ...
    playerModel.Rotate(0, tmpXAxis * Time.deltaTime, 0);
}
    ...
    public IEnumerator Jump()
    {
        ...
        // Trigger jump animation and sound (from PlayerController)
        if (_hasAnimator)
        {
