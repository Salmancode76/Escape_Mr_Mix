    ...
    private void bodyRotation()
{
    if (ingameGlobalManager.instance.mouseWaitUnitlFirstMouseMove)
    {
        ...
        if (!ingameGlobalManager.instance.b_Joystick && mouseHorizontal != 0)
        {
            ...
            // Rotate the player model (Y-axis only)
            playerModel.Rotate(0, tmpXAxis * Time.deltaTime, 0);

            // Rotate the camera's Y-axis only
            Vector3 currentEuler = objCamera.transform.localEulerAngles;
            currentEuler.y += tmpXAxis * Time.deltaTime;
            objCamera.transform.localEulerAngles = new Vector3(0, currentEuler.y, 0);
        }

        // Joystick Case
        else if (ingameGlobalManager.instance.b_Joystick)
        {
            Vector3 rotation = joyInput * animationCurveJoystick.Evaluate(joyInput.magnitude) * ingameGlobalManager.instance.inputListOfFloatGamepadButton[gamepadLookInGameSensibilty] * 100f * Time.deltaTime;
            playerModel.Rotate(0, rotation.y, 0);
            objCamera.localEulerAngles += rotation;
            objCamera.localEulerAngles = new Vector3(ClampAngle(objCamera.localEulerAngles.x, minimum, maximum), objCamera.localEulerAngles.y, 0);
        }
    }
}
