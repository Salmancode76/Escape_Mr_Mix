...
public class characterMovement : MonoBehaviour {
   ...
   //float mouseInputY = 0;
    ...
    private void Start()
    {
        ...
        // Animation initialization (from PlayerController)
    ...
    private void bodyRotation() { 
        if (ingameGlobalManager.instance.mouseWaitUnitlFirstMouseMove) { 
            if (!ingameGlobalManager.instance.b_Joystick && mouseHorizontal != 0) { 
                tmpXAxis = mouseInputX * 1.1f; tmpXAxis *= sensibilityMouse * ingameGlobalManager.instance.inputListOfFloatKeyboardButton[mouseInGameSensibilty]; // Rotate player based on mouse input (adapted from PlayerController) rbBodyCharacter.MoveRotation(rbBodyCharacter.rotation * Quaternion.Euler(0, mouseInputX * sensibilityMouse * Time.deltaTime, 0)); } else { tmpXAxis = 0; }
