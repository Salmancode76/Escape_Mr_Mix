...
//--> Prevent Mac bug with the cursor lockstate
    ...
    Vector3 cameraForward = objCamera.forward;
    Vector3 cameraRight = objCamera.right;
    cameraForward.y = 0; // Keep movement horizontal
    cameraRight.y = 0;
    cameraForward.Normalize();
    cameraRight.Normalize();

    Vector3 Direction = (cameraForward * YAxis) + (cameraRight * XAxis);

    // Add mobile input if needed
    if (b_MoveForward) Direction += cameraForward;
    if (b_MoveBackward) Direction -= cameraForward;
    if (b_MoveRight) Direction += cameraRight;
    if (b_MoveLeft) Direction -= cameraRight;
    ...
    if (isOnFloor && currentAngle >= 180 - MaxAngle) {
        rbBodyCharacter.AddForce(Direction.normalized * characterSpeed * currentSpeedMultiplier, ForceMode.Force);
    }

    // Braking force
    Vector3 opposite = -rbBodyCharacter.velocity.normalized * BrakeForce * Coeff;
    rbBodyCharacter.AddForce(opposite, ForceMode.Force);

    // Animation updates
    if (_hasAnimator) {
        Vector3 localVelocity = transform.InverseTransformDirection(rbBodyCharacter.velocity);
        _currentVelocity.x = Mathf.Lerp(_currentVelocity.x, localVelocity.x, AnimBlendSpeed * Time.fixedDeltaTime);
        _currentVelocity.y = Mathf.Lerp(_currentVelocity.y, localVelocity.z, AnimBlendSpeed * Time.fixedDeltaTime);
