...
//--> Prevent Mac bug with the cursor lockstate
    ...
    private void bodyMovement()
    {
        ...
        // 2. Override with mobile buttons if used
        if (b_MoveForward)  v =  1;
        if (b_MoveBackward) v = -1;
        if (b_MoveRight)    h =  1;
        if (b_MoveLeft)     h = -1;

        // 3. Dead‑zone filter
        h = Mathf.Abs(h) > minimumAxisMovement ? h : 0;
        v = Mathf.Abs(v) > minimumAxisMovement ? v : 0;

        // 4. Normalize diagonal
        Vector2 moveInput = new Vector2(h, v);
        if (moveInput.magnitude > 1f) moveInput.Normalize();

        // 5. Build camera‑relative axes
        ...
        // 6. Compute world‑space direction
        Vector3 Direction = camRight * moveInput.x
                        + camForward * moveInput.y;

        // 7. Apply movement force
        float speed = characterSpeed * currentSpeedMultiplier;
        rbBodyCharacter.AddForce(Direction * speed, ForceMode.Force);

        // 8. Braking when grounded
        if (isOnFloor)
        {
            Vector3 brake = rbBodyCharacter.transform.InverseTransformDirection(-rbBodyCharacter.velocity);
            rbBodyCharacter.AddRelativeForce(brake * BrakeForce * Coeff, ForceMode.Force);
        }
        ...
        // 10. Drive animations
        _currentVelocity.x = Mathf.Lerp(_currentVelocity.x, moveInput.x * speed, AnimBlendSpeed * Time.fixedDeltaTime);
        _currentVelocity.y = Mathf.Lerp(_currentVelocity.y, moveInput.y * speed, AnimBlendSpeed * Time.fixedDeltaTime);
