...
//--> Prevent Mac bug with the cursor lockstate
    ...
    void bodyMovement()
    {
        ...
        // 2. Build world‑space direction from camera
        Vector3 camForward = objCamera.forward;
        camForward.y = 0;
        camForward.Normalize();
        Vector3 camRight = objCamera.right;
        camRight.y = 0;
        camRight.Normalize();

        Vector3 desiredDir = camRight * input.x + camForward * input.y;

        // 3. Determine target speed
        float targetSpeed = characterSpeed * currentSpeedMultiplier;
        if (b_Crouch) targetSpeed *= 0.75f;
        if (input == Vector2.zero) targetSpeed = 0;

        // 4. Compute velocity change
        Vector3 currentVel = rbBodyCharacter.velocity;
        Vector3 currentPlanar = new Vector3(currentVel.x, 0, currentVel.z);
        Vector3 targetVel = desiredDir * targetSpeed;
        Vector3 velocityChange = targetVel - currentPlanar;

        // 5. Apply force for movement
        rbBodyCharacter.AddForce(velocityChange, ForceMode.VelocityChange);

        // 6. Brake or air logic
        if (isOnFloor)
        {
            Vector3 brake = -currentPlanar * BrakeForce * Coeff;
            rbBodyCharacter.AddRelativeForce(brake, ForceMode.Force);
        }

        // 7. Cap max speed
        Vector3 finalVel = rbBodyCharacter.velocity;
        Vector3 planar = new Vector3(finalVel.x, 0, finalVel.z);
        if (planar.magnitude > MaxSpeed)
            planar = planar.normalized * MaxSpeed;
        rbBodyCharacter.velocity = new Vector3(planar.x, finalVel.y, planar.z);

        // 8. Update animation blend
        _currentVelocity.x = Mathf.Lerp(_currentVelocity.x, input.x * targetSpeed, AnimBlendSpeed * Time.fixedDeltaTime);
        _currentVelocity.y = Mathf.Lerp(_currentVelocity.y, input.y * targetSpeed, AnimBlendSpeed * Time.fixedDeltaTime);
