...
public float GetMouseXInput() {
    // Return the current horizontal mouse input based on control scheme
    if (ingameGlobalManager.instance.b_DesktopInputs)
    {
        if (!ingameGlobalManager.instance.b_Joystick)
        {
            // Mouse control
            return mouseHorizontal * sensibilityMouse * ingameGlobalManager.instance.inputListOfFloatKeyboardButton[mouseInGameSensibilty];
        }
        else
        {
            // Joystick control
            return joyHorizontal * animationCurveJoystick.Evaluate(joyInput.magnitude) * 
                   ingameGlobalManager.instance.inputListOfFloatGamepadButton[gamepadLookInGameSensibilty] * 100f * Time.deltaTime;
        }
    }
    else
    {
        ...
        {
            return mobileToystickController.inputVector.x * sensibilityMobile;
        }
        else
        {
            // Touch input (not currently tracked in a variable, would need implementation)
            return 0f;
        }
    }
}
...
public float GetMouseYInput() {
    // Return the current vertical mouse input based on control scheme
    if (ingameGlobalManager.instance.b_DesktopInputs)
    {
        if (!ingameGlobalManager.instance.b_Joystick)
        {
            // Mouse control
            return mouseVertical * sensibilityMouse * returnInvertMouseAxis() * 
                   ingameGlobalManager.instance.inputListOfFloatKeyboardButton[mouseInGameSensibilty];
        }
        else
        {
            // Joystick control
            return joyVertical * animationCurveJoystick.Evaluate(joyInput.magnitude) * 
                   ingameGlobalManager.instance.inputListOfFloatGamepadButton[gamepadLookInGameSensibilty] * 100f * Time.deltaTime;
        }
    }
    else
    {
        ...
        if (b_MobileCamRotation_Stick)
        {
            return mobileToystickController.inputVector.z * sensibilityMobile * returnInvertJoystickAxis();
        }
        else
        {
            // Touch input (not currently tracked in a variable, would need implementation)
