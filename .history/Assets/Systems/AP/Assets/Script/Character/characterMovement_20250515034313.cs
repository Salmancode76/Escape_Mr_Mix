...
public float GetMouseYInput()
{
    ...
    void LateUpdate()
    {
        ...
        // Get player's current Y rotation
        float playerY = transform.eulerAngles.y; // or rbBodyCharacter.rotation.eulerAngles.y if Rigidbody used

        // Get camera rotation and update Y only
        Vector3 camEuler = mainCamera.eulerAngles;
        camEuler.y = playerY;
        mainCamera.eulerAngles = camEuler;
    }
