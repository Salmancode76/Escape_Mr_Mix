...
public class characterMovement : MonoBehaviour
{
    ...
    // public Transform Character; // Reference to the camera
...
public float GetMouseYInput()
{
    ...
    private void bodyRotation()
    {
        if (ingameGlobalManager.instance.mouseWaitUnitlFirstMouseMove)
        {
            ...
            gameObject.transform.Rotate(0, tmpXAxis, 0);
        }
        else if (ingameGlobalManager.instance.b_Joystick)
        {
            transform.localEulerAngles += joyInput * animationCurveJoystick.Evaluate(joyInput.magnitude) * ingameGlobalManager.instance.inputListOfFloatGamepadButton[gamepadLookInGameSensibilty] * 100f * Time.deltaTime;
            transform.localEulerAngles = new Vector3(ClampAngle(transform.localEulerAngles.x, minimum, maximum),
        ...
        else
        {
            for (int i = 0; i < Input.touchCount; ++i)
            {
                ...
                if (Input.GetTouch(i).phase == TouchPhase.Moved)
                {
                    ...
                    {
                        transform.transform.Rotate(Vector3.up * touchDeltaPosition.x * mobileSpeedRotation * Time.deltaTime, Space.World);
