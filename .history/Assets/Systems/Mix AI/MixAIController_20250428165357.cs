...
public class MixAIController : MonoBehaviour
{
        ...
        if (speed > 0.1f)
        {
            ...
            if (footstepTimer >= interval)
            {
                ...
                if (footstepClips.Length > 0)
                {
                    ...
                    SoundFXManager.instance.playSoundFXClip(step, transform, 0.8f);
                }

                if (bassClip != null)
                {
                    SoundFXManager.instance.playSoundFXClip(bassClip, transform, 0.5f);
                }
            }
        }
        else
    ...
    void EndGame()
    {
        ...
        // Here, you can load a "Game Over" scene or quit the game
        SceneManager.LoadScene("GameOver");  // Change "GameOver" to your game over scene name
        // Application.Quit();  // Optionally, you can use this to quit the game if no scene is needed
    }
}
