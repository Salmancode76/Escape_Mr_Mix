...
public class MixAIController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        HandleFootsteps(); // Always handle footsteps
    }
    ...
    void ChasePlayer()
    {
        ...
        // Change the distance check to 0.5f
        if (distance <= 0.5f)
        {
    ...
    void HandleFootsteps()
    {
        ...
        if (speed > 0.1f)
        {
            ...
            if (footstepTimer >= interval)
            {
                ...
                // Always play a footstep sound
                AudioClip footstepClip = footstepClips[Random.Range(0, footstepClips.Length)];
                SoundFXManager.instance.PlayFootstepsBasedOnDistance(footstepClip, transform, 1f);

                // Always play the bass clip alongside the footstep sound
                SoundFXManager.instance.PlayFootstepsBasedOnDistance(bassClip, transform, 0.5f);  // Adjust bass volume as needed
            }
        }
        else
        {
    ...
    void EndGame()
    {
        ...
        // End the session in the editor
        #if UNITY_EDITOR
        UnityEditor.EditorApplication.isPlaying = false;
        #else
        // Optionally: Quit the application if running as a standalone build
        Application.Quit();
        #endif
    }
}
