...
public class MixAIController : MonoBehaviour
{
    ...
    private void HandleChaseState()
    {
        ...
        // Force running state in OnFollow mode
        if (behaviorMode == BehaviorMode.OnFollow)
        {
            hasLineOfSight = true;
        ...
        // INSTANT KILL if extremely close
    ...
    {
        // Always check if we're moving (more reliable than state checks)
        ...
        animator.SetBool("Run", isMoving && shouldRun);
        animator.SetBool("Attack", currentState == AIState.Attacking);

        // Debug information
        Debug.Log($"Movement: {isMoving} | State: {currentState} | Run: {shouldRun} | Velocity: {navAgent.velocity.magnitude}");
    }
