...
public class MixAIController : MonoBehaviour
{
    ...
    {
        // More accurate movement detection using desiredVelocity
        bool isMoving = navAgent.desiredVelocity.magnitude > 0.1f;
        
        bool shouldRun = currentState == AIState.Chasing || 
                    behaviorMode == BehaviorMode.OnFollow;
        
        // Force immediate animation updates
        animator.applyRootMotion = false;
        animator.SetBool("Walk", isMoving && !shouldRun);
        animator.SetBool("Run", shouldRun && isMoving);
        
        // Debug with more details
        Debug.Log($"AnimParams - Moving:{isMoving} State:{currentState} " +
                $"Run:{shouldRun} Vel:{navAgent.velocity.magnitude} " +
                $"DesVel:{navAgent.desiredVelocity.magnitude}");
    }
