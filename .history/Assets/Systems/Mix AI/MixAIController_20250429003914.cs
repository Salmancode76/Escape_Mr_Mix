...
public class MixAIController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (behaviorMode == BehaviorMode.OnFollow && playerTransform != null)
        {
            ...
            return; // Skip patrol setup if in follow mode
        }
    ...
    private void HandleChaseState()
    {
        ...
        // For OnFollow mode, ensure we always have line of sight
        if (behaviorMode == BehaviorMode.OnFollow)
        {
            hasLineOfSight = true;
        }
...
private void InstantKillPlayer()
{
    ...
    private void TransitionToState(AIState newState)
    {
        ...
        // Reset animation parameters during state changes
        if (animator != null)
        {
    ...
    private void UpdateAnimations()
    {
        ...
        bool shouldWalk = isMoving && (currentState == AIState.Patrolling || 
                                        currentState == AIState.Investigating);
        bool shouldRun = isMoving && (currentState == AIState.Chasing || 
                                    behaviorMode == BehaviorMode.OnFollow);

        // Respect your animator's state machine flow
        animator.SetBool("Walk", shouldWalk || shouldRun); // Must be walking before running
        animator.SetBool("Run", shouldRun); // Only set Run when Walk is true
        
        Debug.Log($"Anim States - Walk:{shouldWalk} Run:{shouldRun} Moving:{isMoving}");
    }
