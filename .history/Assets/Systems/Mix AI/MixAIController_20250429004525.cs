...
public class MixAIController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (behaviorMode == BehaviorMode.OnFollow && playerTransform != null)
        {
            ...
            // Force immediate run animation
            if (animator != null)
            {
                animator.Play("Run", 0, 0f); // Immediate transition to run
...
private void InstantKillPlayer()
{
    ...
    private void UpdateAnimations()
    {
        ...
        // Skip walk state completely when we should run
        bool shouldRun = (currentState == AIState.Chasing) || 
                        (behaviorMode == BehaviorMode.OnFollow && currentState != AIState.Idle);
        
        animator.SetBool("Walk", false); // Never use walk state
        animator.SetBool("Run", shouldRun && isMoving);
        
        // For instant transitions
        if (shouldRun && isMoving)
        {
            animator.Play("Run", 0, 0f); // Force immediate transition
        }
    }
