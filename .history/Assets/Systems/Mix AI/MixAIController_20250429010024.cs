...
public class MixAIController : MonoBehaviour
{
    ...
    private void HandleChaseState()
    {
        ...
        // Calculate direct distance (not path distance)
        float directDistance = Vector3.Distance(transform.position, playerTransform.position);
        
        // INSTANT KILL if extremely close (1 unit or less)
        if (directDistance <= 1f)
        {
            if (CheckDirectLineOfSight())
            {
                InstantKillPlayer();
                return;
            }
        ...
        // Normal detection and chasing logic
        if (hasLineOfSight)
        {
            lastKnownPlayerPosition = playerTransform.position;
            losePlayerTimer = 0;
        ...
        // Check if Mix is stuck near the player
        CheckIfStuck(directDistance);
    }
    ...
    private bool CheckDirectLineOfSight()
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(transform.position + Vector3.up * 0.5f, direction.normalized, out hit, distance, obstacleLayer))
        {
    ...
    private void CheckIfStuck(float directDistance)
    {
        // Check if Mix AI is very close to the player and hasn't moved significantly
        if (directDistance <= 1f && navAgent.velocity.magnitude < 0.1f)
        {
            // Start a coroutine to wait before registering the player as dead
            StartCoroutine(WaitAndRegisterKill());
        }
    }

    private IEnumerator WaitAndRegisterKill()
    {
        // Wait for 3 seconds
        yield return new WaitForSeconds(3f);
        
        // Check again if the player is still within kill range
    ...
    private void InstantKillPlayer()
    {
        Debug.Log("INSTANT KILL! Mix caught player at point-blank range");
        
        // Play kill animation/sound if needed
        if (attackSound != null)
