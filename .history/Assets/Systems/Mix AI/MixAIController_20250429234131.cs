...
public class MixAIController : MonoBehaviour
{
    ...
    private void HandleChaseState()
    {
        ...
        {
            Debug.LogWarning("Player transform null - returning to patrol");
            TransitionToState(AIState.Patrolling);
            return;
        }
        ...
        // Debug movement info
        Debug.Log($"Chasing player at speed: {navAgent.speed}");
        Debug.Log($"Distance to player: {Vector3.Distance(transform.position, playerTransform.position)}");
        Debug.Log($"Agent velocity: {navAgent.velocity.magnitude}");

        // Force run animation
        if (animator != null)
        {
            Debug.Log("Setting run animation");
            animator.SetBool("Run", true);
            animator.SetBool("Walk", false);
            animator.speed = 1.5f;
        }
        ...
        {
            Debug.Log("Player in kill range - instant kill");
            InstantKillPlayer();
            return;
        }
    ...
    private void DetectPlayer()
    {
        ...
        {
            bool isPlayerMakingNoise = behaviorMode == BehaviorMode.OnDetect ? 
                IsPlayerRunning : 
                (distanceToPlayer <= effectiveHearingRadius * 0.7f);
    ...
    private void TransitionToState(AIState newState)
    {
        ...
        {
            Debug.Log($"State change too fast - ignoring transition from {currentState} to {newState}");
            return;
        }
        ...
        switch (newState)
        {
            ...
            case AIState.Chasing:
                ...
                if (animator != null)
                {
                    animator.Play("Run", 0, 0f); // Force run animation
                    animator.SetBool("Run", true);
                    animator.SetBool("Walk", false);
                    animator.speed = 1.5f;
                }
    ...
    private void HandleFootsteps()
    {
        ...
        if (speed > 0.1f)
        {
            ...
            // Double footstep rate when chasing
            float interval = (currentState == AIState.Chasing) ? 
                runFootstepRate * 0.5f : 
                walkFootstepRate;
    ...
    private void UpdateAnimations()
    {
        ...
        Debug.Log($"UpdateAnimations - isMoving: {isMoving}, shouldRun: {shouldRun}, currentState: {currentState}");

        if (currentState == AIState.Chasing)
        {
            Debug.Log("Forcing chase animations");
