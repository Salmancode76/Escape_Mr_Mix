...
public class MixAIController : MonoBehaviour
{
    ...
    private void HandleChaseState()
    {
        ...
        // Force run animation exactly like in OnFollow mode
        if (animator != null)
        {
            animator.SetBool("Walk", false);
            animator.SetBool("Run", true);
            animator.speed = 1f; // Match OnFollow animation speed
        }
    ...
    private void TransitionToState(AIState newState)
    {
        ...
        // Reset previous state parameters
        if (animator != null)
        {
            animator.SetBool("Walk", false);
            animator.SetBool("Run", false);
        ...
        switch (newState)
        {
            case AIState.Idle:
                ...
                if (animator != null)
                {
                    animator.Play("Idle", 0, 0f); // Force idle state
                }
                break;
            case AIState.Patrolling:
                navAgent.speed = patrolSpeed;
    ...
    {
        // For chase state, we handle everything in TransitionToState
        ...
        if (animator != null)
        {
            if (currentState == AIState.Patrolling || currentState == AIState.Investigating)
            {
