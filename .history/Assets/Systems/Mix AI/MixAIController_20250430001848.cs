...
public class MixAIController : MonoBehaviour
{
    ...
    // Properties to check player state - only used in OnDetect mode
    ...
    public AudioClip DeadPlayerSFX, DeathUIMusicSFX; // Assign a kill sound clip here
    ...
    private void Awake()
    {
        ...
        // Get player transform, which is needed for all modes
        playerTransform = playerObject.transform;
        
        // Only get PlayerController and InputManager if using OnDetect mode
        if (behaviorMode == BehaviorMode.OnDetect)
        {
            playerController = playerObject.GetComponent<PlayerController>();
            inputManager = playerObject.GetComponent<InputManager>();

            // Only validate these components for OnDetect mode
            if (playerController == null)
            {
                Debug.LogError("PlayerController component missing on player object! Required for OnDetect mode.");
            }

            if (inputManager == null)
            {
                Debug.LogError("InputManager component missing on player object! Required for OnDetect mode.");
            }
        }
    ...
    private void FindPlayer()
    {
        ...
        if (player != null)
        {
            ...
            // Only get these components if in OnDetect mode
            if (behaviorMode == BehaviorMode.OnDetect)
            {
    ...
    private void Update()
    {
        ...
        if (behaviorMode == BehaviorMode.OnFollow && playerTransform != null)
        {
            ...
            // Ensure we're in chase state when in OnFollow mode
    ...
    private void HandleChaseState()
    {
        ...
        // Update destination every frame - more aggressive in OnFollow mode
        if (behaviorMode == BehaviorMode.OnFollow)
        {
            // Direct path with no path recalculation delay
            navAgent.SetDestination(playerTransform.position);
            navAgent.acceleration = 20f; // More responsive movement
        }
        else
        {
            // Original chasing behavior for OnDetect mode
            navAgent.SetDestination(playerTransform.position);
        }
    ...
    public void InstantKillPlayer()
    {
        ...
        {
            Debug.LogWarning("Death UI Canvas not assigned in Inspector!");
        }
    ...
    private void DetectPlayer()
    {
        ...
        // Only use stealth modifiers if we're in OnDetect mode and have InputManager
        if (behaviorMode == BehaviorMode.OnDetect && inputManager != null)
        {
        ...
        if (distanceToPlayer <= effectiveHearingRadius)
        {
            ...
            // Only check if player is running in OnDetect mode
            if (behaviorMode == BehaviorMode.OnDetect && inputManager != null)
            {
            ...
            {
                // In other modes, assume basic noise detection
    ...
    private bool CheckLineOfSight()
    {
        ...
        if (Physics.Raycast(transform.position + Vector3.up * 0.5f, directionToPlayer.normalized, out hit, distanceToPlayer, obstacleLayer | playerLayer))
        {
            ...
            {
                // Only use stealth calculation in OnDetect mode
                if (behaviorMode == BehaviorMode.OnDetect && inputManager != null && IsPlayerCrouching)
                {
    ...
    private void OnPlayerDetected()
    {
        if (currentState != AIState.Chasing && currentState != AIState.Attacking)
        {
            ...
            // Switch to OnFollow behavior when player is detected
            behaviorMode = BehaviorMode.OnFollow;
            TransitionToState(AIState.Chasing);
            
            // Animation handling for immediate chase
            if (animator != null)
            {
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        // Only modify display radius in editor if we have valid input manager
        if (behaviorMode == BehaviorMode.OnDetect && inputManager != null)
        {
            if (IsPlayerCrouching) 
            {
