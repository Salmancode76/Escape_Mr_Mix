...
public class MixAIController : MonoBehaviour
{
    ...
    private void BecomeAwareOfPlayer()
    {
        ...
        // Force chasing state for full sprint logic
        if (behaviorMode == BehaviorMode.OnDetect || behaviorMode == BehaviorMode.OnMahdiLevel)
        {
            OnPlayerDetected(); // <- triggers full sprint logic
            return;
        }
    ...
    private bool isDead = false; // New flag to track if the death sound has played

    private void OnPlayerDetected()
    {
        if (isDead) return; // Prevent further sound effects if player has died already

        // Continue with the normal detection logic
        if (currentState != AIState.Chasing)
        {
        ...
        isDead = true; // Mark as dead after the death sound plays
    }
