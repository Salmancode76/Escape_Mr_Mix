...
namespace UnityTutorial.PlayerControl
{
    ...
    {
        // Public flag to enable/disable input
        ...
        {
            if (!isMoving || _inputManager.Crouch || isSliding)
            {
                footstepTimer = 0f;
        ...
        private void PlayNextFootstep()
        {
            ...
            {
                clipToPlay = footstepClips[randomSequence[currentStepIndex++]];
                if (currentStepIndex >= footstepClips.Length)
                {
                    currentStepIndex = 0;
                    randomSequence = GenerateRandomSequence();
            ...
            // Adjust volume when crouching
            float volume = _inputManager.Crouch ? 0.7f : 1f;
        ...
        {
            if (!_hasAnimator || !_inputManager.Jump || !_grounded || !InputEnabled) return;

            _animator.SetTrigger(_jumpHash);
            SoundFXManager.instance.playSoundFXClip(jumpClip, transform, 1f); // Play jump sound

            hasPlayedLanding = false; // Reset landing flag when a new jump occurs
        }
        ...
        private bool hasPlayedLanding = false; // Track landing sound state
        ...
        private void SampleGround()
        {
            ...
            if (Physics.Raycast(_playerRigidbody.worldCenterOfMass, Vector3.down, out hitInfo, Dis2Ground + 0.1f, GroundCheck))
            {
                ...
                // Play landing sound only once after jumping or falling, even if crouching
                if (!hasPlayedLanding && !isSliding)
                {
                    SoundFXManager.instance.playSoundFXClip(landingClip, transform, 1f);
                    hasPlayedLanding = true; // Ensure it plays only once
                }
                return;
            }
            ...
            hasPlayedLanding = false; // Reset when the player is in the air
        }
