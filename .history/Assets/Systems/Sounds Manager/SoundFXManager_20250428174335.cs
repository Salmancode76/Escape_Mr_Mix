...
public class SoundFXManager : MonoBehaviour
{
    ...
    public AudioSource ForMixplaySoundFXClip(AudioClip audioClip, Transform spawnTransform, float volume)
    {
        // Spawn the audio source
        AudioSource audioSource = Instantiate(soundFXObject, spawnTransform.position, quaternion.identity);
        ...
        // Return the audio source for further control (manual stopping)
        return audioSource;
    }
    ...
    public void PlayFootstepsBasedOnDistance(Transform mixTransform, Transform playerTransform, AudioClip[] footstepClips, AudioClip bassClip, float minDistance = 1f, float maxDistance = 25f)
    {
        ...
        // If the player is within range (minDistance to maxDistance)
        if (distance <= maxDistance)
        {
            ...
            if (footstepClips.Length > 0)
            {
                ...
                AudioSource audioSource = ForMixplaySoundFXClip(step, mixTransform, volume); // Adjusted volume and using the method that returns audioSource
                ...
                if (audioSource != null)
                {
                    ...
                    audioSource.maxDistance = maxDistance; // Set the maximum distance to 25
                }
            }
            ...
            if (bassClip != null)
            {
                AudioSource audioSource = ForMixplaySoundFXClip(bassClip, mixTransform, volume); // Adjusted volume and using the method that returns audioSource
