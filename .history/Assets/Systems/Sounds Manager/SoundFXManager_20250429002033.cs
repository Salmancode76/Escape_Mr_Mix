...
public class SoundFXManager : MonoBehaviour
{
    ...
    public void PlayFootstepsBasedOnDistance(AudioClip audioClip, Transform spawnTransform, float volume)
    {
        ...
        // Assign volume
        audioSource.volume = volume;
        audioSource.rolloffMode = AudioRolloffMode.Linear; // Set rolloff mode to linear for 3D sound
        audioSource.spatialBlend = 1f; // Set to 3D sound
        audioSource.maxDistance = 25f; // Set max distance for 3D sound
        audioSource.minDistance = 6f; // Set min distance for 3D sound
        // Play sound
        audioSource.Play();
