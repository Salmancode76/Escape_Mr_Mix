...
public class Menu : MonoBehaviour {
	...
	public void OnNewGamePick(int slotIndex) {
		...
		if (PlayerPrefs.GetInt("Saved" + slotIndex) == 0) {
			// Save initial scene index (1) and dummy time
			...
			// Start from scene 1
			SaveLoadManager.instance.SaveGame(slotIndex); // Save current state
			SaveLoadManager.instance.LoadSceneAsync(1);   // Load first scene
		} else {
			OnBack(); // Slot already used, go back
		}
	}
    ...
    PlayerPrefs.DeleteKey("Saved" + slotIndex);
    PlayerPrefs.DeleteKey("SceneIndex" + slotIndex);
    PlayerPrefs.DeleteKey("TimeSave" + slotIndex);
	}
