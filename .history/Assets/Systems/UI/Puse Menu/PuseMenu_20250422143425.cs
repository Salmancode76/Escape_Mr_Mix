...
public class PuseMenu : MonoBehaviour {
	...
	private void Awake()
	{
		// If instance is already set, don't do anything
		if (instance == null)
		{
			instance = this;
			DontDestroyOnLoad(gameObject); // Keeps PuseMenu between scenes
		}
		else
		{
			// If another instance exists, disable the new one
			gameObject.SetActive(false); // Keeps the menu disabled until the proper instance is loaded
		}
	}
