...
public class PuseMenu : MonoBehaviour {
    ...
    // Added variables for pause management
    private float savedTimeScale = 1f;
    private bool isPauseInProgress = false;
    private float pauseDebounceTime = 0.3f;
	...
	void Start () {
		...
		if (PlayerPrefs.GetFloat ("SliderResolut") == 2) {
			Screen.SetResolution(1600, 900, true); // Fixed from 1900 to 900
		}
    ...
    public void PauseGame()
    {
        ...
        StartCoroutine(PauseWithDebounce());
    }
    ...
    // Toggle pause with debounce to prevent flickering
    public void TogglePause()
    {
        if (isPauseInProgress)
        {
            Debug.Log("Pause operation already in progress, ignoring request");
            return;
        }
    ...
    private IEnumerator PauseWithDebounce()
    {
        isPauseInProgress = true;
        Debug.Log("Starting pause toggle operation");
        
        // Toggle pause logic with improved handling
        if (Time.timeScale > 0)
        {
            // Save current time scale before pausing
            savedTimeScale = Time.timeScale;
            
            // First modify time scale
            Time.timeScale = 0f;
            
            // Then show UI after a small delay
        ...
        else
        {
            ...
            // Then restore time scale after a small delay
            yield return new WaitForSecondsRealtime(0.05f);
            Time.timeScale = savedTimeScale;
        }
        
        // Wait for debounce period
        yield return new WaitForSecondsRealtime(pauseDebounceTime);
        
        isPauseInProgress = false;
        Debug.Log("Pause toggle operation completed");
    }
	...
	public void OnExit() {
		// Check if SoundFXManager instance exists before using it
		if (SoundFXManager.instance != null) {
			SoundFXManager.instance.playSoundFXClip(ClickClip, transform, 1f);
		}
		
		// Check if SaveLoadManager instance exists before using it
		if (SaveLoadManager.instance != null) {
			SaveLoadManager.instance.SaveCurrentProgress();
		} else {
			Debug.LogWarning("SaveLoadManager instance is null, cannot save progress before exit");
		}
	...
	public void OnObvodka (Text _text) {
		if (_text == null) {
			Debug.LogWarning("Text parameter is null in OnObvodka");
			return;
		}
    ...
    public void OnSaveGame () {
		if (SaveLoadManager.instance != null) {
			...
			Debug.LogWarning("SaveLoadManager instance is null, cannot save game");
		}
	}
	...
	public void LoadNextLevel(){
		if (HandlePuse.instance != null) {
			HandlePuse.instance.EnableUI();
		} else {
			Debug.LogWarning("HandlePuse.instance is null");
		}
