...
public class AP_Cam_Follow : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if(target != null){
            ...
            transform.rotation = Quaternion.Lerp(transform.rotation, Head.rotation, Time.deltaTime * rotationDamping); 
        }
    ...
    void FollowTarget()
    {
        if (characterMovementScript.b_Crouch)
        {
            transform.position = target.position;
        }else{
            // transform.position = target.position;
            transform.position = Vector3.Lerp(transform.position, Head.position, Time.deltaTime * rotationDamping );
            transform.rotation = Quaternion.Lerp(transform.rotation, Head.rotation, Time.deltaTime * rotationDamping ); 
        }
