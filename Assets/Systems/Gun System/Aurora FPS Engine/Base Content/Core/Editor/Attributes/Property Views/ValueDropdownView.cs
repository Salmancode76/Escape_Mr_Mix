...
namespace AuroraFPSEditor.Attributes
{
    ...
    public sealed class ValueDropdownView : PropertyView
    {
        ...
        public override void OnInitialize(SerializedProperty property, ViewAttribute viewAttribute, GUIContent label)
        {
            ...
            if (ApexReflection.TryDeepFindMethods(target.GetType(), dropdownValuesAttribute.ienumerable, out MethodInfo[] methods))
            {
                for (int i = 0; i < methods.Length; i++)
                {
                    MethodInfo method = methods[i];
                    if (method.Name == dropdownValuesAttribute.ienumerable)
                    {
                        ienumerableMethod = methods[i];
                    }
                }
            }
        }
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (GUI.Button(position, nameofObjectValue, EditorStyles.popup))
            {
                ...
                int count = 0;
                IEnumerable enumerable = ienumerableMethod.Invoke(target, null) as IEnumerable;
