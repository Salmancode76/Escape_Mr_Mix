...
namespace AuroraFPSRuntime.SystemModules.ControllerSystems
{
    ...
    public class PlayerRigidbodyController : PlayerController
    {
        ...
        private void ToggleCrouch()
        {
            if (isCrouching)
            {
                EditBounds(new Vector3(0f, 0.9f, 0f), 1.8f); // Stand up
                isCrouching = false;
            }
            else
            {
                EditBounds(new Vector3(0f, 0.45f, 0f), 0.9f); // Crouch down
                isCrouching = true;
            }
        }

        /// <summary>
        /// Switch capsule collider physic material based on controller state.
        /// </summary>
